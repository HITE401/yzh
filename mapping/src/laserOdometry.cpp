...
Eigen::Map<Eigen::Quaterniond> q_last_curr(para_q);    //q^last_curr
...
void TransformToStart(PointType const *const pi, PointType *const po)
{
    ...
    if (DISTORTION)
        ...
        s = 1.0;
    //s = 1;
    Eigen::Quaterniond q_point_last = Eigen::Quaterniond::Identity().slerp(s, q_last_curr);   //球形插值
...
int main(int argc, char **argv)
{
    ...
    string LOAM_OUT_PATH="/home/yzh/output/loam_odm.csv";
    ...
    while (ros::ok())
    {
        ...
        if (!cornerSharpBuf.empty() && !cornerLessSharpBuf.empty() && !surfFlatBuf.empty() && !surfLessFlatBuf.empty() && !fullPointsBuf.empty())
        {
            ...
            fout.precision(0);
            fout<<laserOdometry.header.stamp.toSec() * 1e09<<",";
            fout.precision(5);
            fout << t_w_curr.x() << ","
              << t_w_curr.y() << ","
              << t_w_curr.z() << ","
	        ...
	        // printf("t_w_curr  x:%f, y:%f, z:%f\n",t_w_curr.x(),t_w_curr.y(),t_w_curr.z());



            // transform corner features and plane features to the scan end point
            //为0因为它不需要变换了，它已经是平移到末尾的点云了
            if (0)
            {
